...
const Header = ({ user }: HeaderProps) => {
    ...
    const navItems: NavItem[] = [
        ...
        { path: 'https://wargame.swua.kr', label: t('nav.wargame'), external: true },
    ]
    ...
    const isActiveNavItem = (item: NavItem) => !item.external && pathname.startsWith(item.path)

    const navClass = (item: NavItem) => `inline-flex h-10 items-center gap-1.5 px-3 text-sm transition ${isActiveNavItem(item) ? 'text-accent font-semibold border-b-2 border-accent' : 'text-text-subtle hover:text-text'}`

    const mobileNavClass = (item: NavItem) => `flex items-center justify-between rounded-md px-3 py-2 text-sm transition ${isActiveNavItem(item) ? 'bg-accent/10 text-accent' : 'text-text-muted hover:bg-surface-muted dark:text-text'}`
            ...
            <header className='bg-surface border-b border-border'>
                <div className='mx-auto flex h-14 w-full max-w-7xl items-center justify-between gap-4 px-4 md:px-6'>
                        ...
                        <nav className='hidden items-center gap-2 lg:flex'>
                            {navItems.map((item) => (
                                <a
                                    ...
                                    onClick={item.external ? undefined : (event) => navigate(item.path, event)}
                                >
                                    <span>{item.label}</span>
                                    {item.external ? <ExternalLinkIcon className='h-3 w-3 opacity-70' /> : null}
                                </a>
                            ))}
                        </nav>
                    </div>
                    ...
                    <div className='hidden items-center gap-2 lg:flex'>
                        <button
                            className='bg-surface-muted px-2 py-1 text-xs text-text-muted transition hover:bg-surface-subtle dark:text-text-muted dark:hover:bg-surface-muted rounded-md'
                            onClick={toggleTheme}
                        ...
                        {user ? (
                            <>
                                ...
                                <div className='relative' ref={profileMenuRef}>
                                    <button
                                        type='button'
                                        className='inline-flex items-center justify-between gap-2.75 rounded-md px-3 py-1.5 text-xs text-text transition hover:bg-surface-muted'
                                        onClick={() => {
            ...
            <aside className={`fixed left-0 top-0 z-50 h-full w-[82vw] max-w-xs transform bg-surface shadow-xl transition-transform duration-200 lg:hidden dark:bg-surface ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <nav className='p-2'>
                    {navItems.map((item) => (
                        ...
                        >
                            <span>{item.label}</span>
                            {item.external ? <ExternalLinkIcon className='h-3.5 w-3.5 opacity-70' /> : null}
                        </a>
                    ))}
                    {user?.role === 'admin' ? (
                        <a
