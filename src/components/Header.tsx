        ...
        <>
            <header className='border-b border-border bg-surface/70 backdrop-blur'>
                <div className='mx-auto flex max-w-6xl items-center justify-between px-6 py-4'>
                    <button
                        className='flex items-center justify-center p-2 text-text lg:hidden cursor-pointer'
                    ...
                    <a
                        href='/'
                        className='hidden items-center gap-4 lg:flex cursor-pointer'
                        onClick={(event) => navigate('/', event)}
                    >
                        <img
                            src={`/logo_${toggleThemeValue(theme)}_cropped.svg`}
                            alt={t('header.logoAlt')}
                            className='h-6 w-auto'
                        />
                        <div>
                            <p className='font-display text-xl text-text'>{appConfig.header_title}</p>
                            <p className='text-xs text-text-muted'>{appConfig.header_description}</p>
                        </div>
                    </a>
                    ...
                    <div className='hidden items-center gap-3 lg:flex'>
                        {user ? (
                            <>
                                <button
                                    className='hidden text-right text-xs text-text-muted sm:block cursor-pointer'
                                ...
                                <button
                                    className='rounded-full border border-border px-4 py-2 text-xs text-text transition hover:border-accent hover:text-accent cursor-pointer'
                                    onClick={() => logout()}
                                >
                                    {t('auth.logout')}
                                </button>
                            ...
                            <>
                                ...
                                <a
                                    href='/register'
                                    className='rounded-full bg-accent/20 px-4 py-2 text-xs text-accent-strong transition hover:bg-accent/30 cursor-pointer'
            ...
            {mobileMenuOpen ? (
                <button
                    className='fixed inset-0 z-40 bg-overlay/50 backdrop-blur-sm lg:hidden cursor-pointer'
            ...
            <aside
                className={`fixed left-0 top-0 z-50 h-full w-72 transform border-r border-border bg-surface shadow-xl transition-transform duration-300 lg:hidden ${
                    ...
                    <div className='flex flex-1 flex-col overflow-y-auto p-6'>
                        {user ? (
                            <div className='mb-6 rounded-lg border border-border bg-surface-muted p-4'>
                                ...
                                {user.role === 'admin' ? (
                                    <span className='mt-2 inline-block rounded-full bg-accent/20 px-2 py-0.5 text-xs text-accent-strong'>
                                        {t('common.admin')}
                                    </span>
                                ) : null}
                            </div>
                        ...
                        <nav className='flex flex-col gap-2'>
                            <a
                                href='/challenges'
                                className='rounded-lg px-4 py-3 text-sm text-text transition hover:bg-accent/10 hover:text-accent cursor-pointer'
                        ...
                        <div className='flex flex-col gap-3'>
                            ...
                            <button
                                className='flex items-center justify-between rounded-lg border border-border px-4 py-3 text-sm text-text transition hover:border-accent hover:text-accent cursor-pointer'
                                onClick={toggleTheme}
                            >
                            ...
                            {user ? (
                                <button
                                    className='rounded-lg border border-danger/40 bg-danger/10 px-4 py-3 text-sm text-danger transition hover:border-danger/50 hover:bg-danger/20 cursor-pointer'
                                ...
                                <>
                                    ...
                                    <a
                                        href='/register'
                                        className='rounded-lg bg-accent/20 px-4 py-3 text-center text-sm text-accent-strong transition hover:bg-accent/30 cursor-pointer'
