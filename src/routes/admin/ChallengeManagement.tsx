...
const ChallengeManagement = () => {
    ...
    const [editVMEnabled, setEditVMEnabled] = useState(false)
    const [editVMSpec, setEditVMSpec] = useState('')
    const [loadedVMSpec, setLoadedVMSpec] = useState('')
    ...
    const openEditor = async (challenge: Challenge) => {
        ...
        setEditVMEnabled('vm_enabled' in challenge ? challenge.vm_enabled : false)
        setEditVMSpec('')
        setLoadedVMSpec('')
        ...
        try {
            ...
            setEditVMEnabled(detail.vm_enabled)
            const specValue = detail.vm_spec ?? ''
            setEditVMSpec(specValue)
            setLoadedVMSpec(specValue)
    ...
    const cancelEdit = (field: typeof editingField, challenge: Challenge) => {
        ...
        if (field === 'vm') {
            setEditVMEnabled(detail?.vm_enabled ?? false)
            setEditVMSpec(loadedVMSpec)
        }
    }
    ...
    const saveField = async (challenge: Challenge, field: typeof editingField) => {
        ...
        if (field === 'vm') {
            const vmChanged = editVMEnabled !== (detail?.vm_enabled ?? false) || editVMSpec !== loadedVMSpec
            ...
            payload.vm_enabled = editVMEnabled
        ...
        try {
            ...
            setEditVMEnabled(updated.vm_enabled)
            if (!updated.vm_enabled) {
                setEditVMSpec('')
                setLoadedVMSpec('')
            } else {
                setLoadedVMSpec(editVMSpec)
            }
            setEditFlag('')
            setEditingField(null)
        } catch (error) {
                                                            ...
                                                            <div className='rounded-2xl border border-border bg-surface/60 p-4'>
                                                                ...
                                                                {editingField === 'vm' ? (
                                                                            ...
                                                                            <div className='grid gap-4'>
                                                                                <div>
                                                                                    ...
                                                                                    {manageFieldErrors.vm_spec ? (
                                                                                        <p className='mt-2 text-xs text-danger'>
                                                                                            {t('admin.create.spec')}: {manageFieldErrors.vm_spec}
                                                                                        </p>
                                                                                    ) : null}
                                                                                </div>
                                                                            </div>
                                                                    ...
                                                                    <div className='mt-3 space-y-1 text-sm text-text'>
                                                                        <p>{editVMEnabled ? t('common.active') : t('common.inactive')}</p>
                                                                        {editVMEnabled ? (
                                                                            <p>
                                                                                {t('admin.create.spec')}: {loadedVMSpec ? t('admin.manage.specConfigured') : t('admin.manage.specMissing')}
                                                                            </p>
                                                                        ) : null}
                                                                    </div>
                                                                )}
